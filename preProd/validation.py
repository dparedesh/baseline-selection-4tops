...
def plot(name):
    ...
    for i, f in enumerate(files):
        h_orig = f.Get(name)
        if not h_orig:
           print("Could not find", name, "in", f)
           continue
...
channels = ['SSee', 'SSmm', 'SSem', 'eee', 'eem', 'emm', 'mmm', 'eee_Zveto', 'eem_Zveto', 'emm_Zveto', 'mmm_Zveto']
...
for channel in channels:
    plot(channel + '/cutflow_Loose')

for channel in channels:
    h1(channel)

    h3('Event stuff')
    plot(channel + '/cutflow_mc_Loose')
    plot(channel + '/cutflow_pu_Loose')
    plot(channel + '/cutflow_zvtx_Loose')
    plot(channel + '/cutflow_scale_factors_Loose')
    plot(channel + '/cutflow_btag_scale_factors_Loose')
